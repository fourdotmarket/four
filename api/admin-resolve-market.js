...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Get request data
    const { market_id, outcome, resolution_reason } = req.body;

    if (market_id === undefined || outcome === null || !resolution_reason) {
    ...
    // Log admin action
    console.log(`✅ ADMIN RESOLUTION: Market #${market_id} resolved by ${user.username}`);
    console.log(`   Question: ${market.question}`);
    console.log(`   Outcome: ${outcome ? 'YES (Maker wins)' : 'NO (Challengers win)'}`);
    console.log(`   Reason: ${resolution_reason}`);
    ...
    console.error('Admin resolve market error:', error.message);
